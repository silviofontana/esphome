...
void LM75Sensor::dump_config() {
  LOG_SENSOR("", "LM75 Sensor", this);
  if (this->is_failed()) {
...
void LM75Sensor::set_fault_count(const int faults) {
  if (faults < 1) {
...
void LM75Sensor::write_config() {
  send_alert_limit_low_();
  send_alert_limit_high_();
  send_config_();
}

void LM75Sensor::send_config_() {
...
void LM75Sensor::send_alert_limit_low_() {
...
void LM75Sensor::send_alert_limit_high_() {
